...
// ✅ handleRegister จริง เชื่อม Supabase
async function handleRegister() {
  ...
  try {
    ...
    console.log('authUser:', authUser)           // ดูว่ามี id ไหม
    console.log('token:', auth.getToken())        // ดูว่ามี token ไหม
    if (authUser?.id) {
      const result = await db.updateUserProfile(authUser.id, {
        full_name: name.value,
        phone: phone.value || undefined,
        bio: bio.value || undefined,
        skills: selectedSkills.value.length ? selectedSkills.value : undefined,
      } as any)
      console.log('update result:', result)       // ดูว่า update สำเร็จไหม
    }
    navigateTo('/login')
  } catch (e: any) {
  ...
  <div class="min-h-screen flex">
    ...
    <div class="hidden lg:flex w-1/2 flex-col justify-between bg-emerald-600 p-14 text-white relative overflow-hidden">
      ...
      <div class="relative flex items-center gap-2.5">
        ...
        <span class="font-semibold text-xl tracking-tight">VolunteerSpace</span>
      </div>
    ...
    <div class="flex-1 flex items-start justify-center bg-stone-50 px-8 py-12 overflow-y-auto min-h-screen">
      <div class="w-full max-w-[440px]">
        ...
        <NuxtLink to="/"
          class="inline-flex items-center gap-1.5 text-sm text-stone-400 hover:text-stone-700 transition-colors mb-8">
        ...
        <!-- Error global -->
        <div v-if="errorMsg" class="mb-5 px-4 py-3 bg-red-50 border border-red-100 rounded-xl text-sm text-red-600">
          {{ errorMsg }}
        </div>
          ...
          <div class="flex flex-col gap-4">
            ...
            <div class="flex flex-col gap-1.5">
              ...
              <div class="relative">
                ...
                <button type="button" class="absolute right-3 top-1/2 -translate-y-1/2 text-stone-300 hover:text-stone-500 p-1"
        ...
        <div v-else-if="step === 2">
          ...
          <p class="text-sm text-stone-400 mb-6">ขั้นตอนที่ 2/3 · กรอกหรือข้ามได้ แก้ไขภายหลังได้เสมอ</p>

          <div class="flex flex-col gap-4">
            <div class="flex flex-col gap-1.5">
              <label class="text-sm font-medium text-stone-700">เบอร์โทรศัพท์</label>
              <input v-model="phone" type="tel" placeholder="081-234-5678"
            ...
            <div class="flex flex-col gap-1.5">
              ...
              <textarea v-model="address" rows="3" placeholder="123 ถ.สุขุมวิท แขวงคลองเตย กรุงเทพฯ 10110"
            ...
            <div class="flex flex-col gap-1.5">
              ...
              <textarea v-model="bio" rows="3" placeholder="เล่าให้ฟังหน่อยว่าทำไมถึงอยากทำจิตอาสา..."
                class="px-4 py-3 text-sm border border-stone-200 rounded-xl bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-400 transition resize-none placeholder:text-stone-300" />
              <p class="text-xs text-stone-400 self-end">{{ bio.length }} ตัวอักษร</p>
            </div>
          </div>
        </div>
        ...
        <div v-else-if="step === 3">
          ...
          <p class="text-sm text-stone-400 mb-6">ขั้นตอนที่ 3/3 · เลือกได้หลายข้อ ช่วยจับคู่กิจกรรมได้ดีขึ้น</p>

          <!-- Skills -->
          <div class="mb-6">
            <div class="flex items-center justify-between mb-2">
              <label class="text-sm font-medium text-stone-700">ทักษะของคุณ</label>
              <span class="text-xs text-emerald-600 font-medium">{{ selectedSkills.length }} ทักษะ</span>
            </div>
            <div class="flex flex-wrap gap-2 mb-3">
              <button v-for="s in SKILL_OPTIONS" :key="s" type="button"
                class="text-xs px-3 py-1.5 rounded-full border transition-all"
                :class="selectedSkills.includes(s) ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-white text-stone-600 border-stone-200 hover:border-emerald-300'"
                @click="toggleSkill(s)">{{ s }}</button>
            </div>
            <div class="flex gap-2">
              <input v-model="customSkill" type="text" placeholder="เพิ่มทักษะอื่นๆ..."
              ...
              <button type="button" class="px-3 py-2 text-sm bg-stone-100 hover:bg-stone-200 text-stone-600 rounded-xl"
                @click="addCustomSkill">+ เพิ่ม</button>
            </div>
            <div v-if="selectedSkills.some(s => !SKILL_OPTIONS.includes(s))" class="flex flex-wrap gap-1.5 mt-2">
              <span v-for="s in selectedSkills.filter(s => !SKILL_OPTIONS.includes(s))" :key="s"
                ...
                {{ s }}
                <button type="button" class="hover:text-red-500" @click="toggleSkill(s)">×</button>
              </span>
            </div>
          </div>
          ...
          <div>
            <div class="flex items-center justify-between mb-2">
              <label class="text-sm font-medium text-stone-700">ประเภทกิจกรรมที่สนใจ</label>
              <span class="text-xs text-amber-600 font-medium">{{ selectedInterests.length }} ประเภท</span>
            </div>
            <div class="flex flex-wrap gap-2 mb-3">
              <button v-for="it in INTEREST_OPTIONS" :key="it" type="button"
                class="text-xs px-3 py-1.5 rounded-full border transition-all"
                :class="selectedInterests.includes(it) ? 'bg-amber-500 text-white border-amber-500' : 'bg-white text-stone-600 border-stone-200 hover:border-amber-300'"
                @click="toggleInterest(it)">{{ it }}</button>
            </div>
            <div class="flex gap-2">
              <input v-model="customInterest" type="text" placeholder="เพิ่มความสนใจอื่นๆ..."
                class="flex-1 px-3 py-2 text-sm border border-stone-200 rounded-xl bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500/30 placeholder:text-stone-300"
                @keyup.enter="addCustomInterest" />
              <button type="button" class="px-3 py-2 text-sm bg-stone-100 hover:bg-stone-200 text-stone-600 rounded-xl"
                @click="addCustomInterest">+ เพิ่ม</button>
            </div>
            <div v-if="selectedInterests.some(i => !INTEREST_OPTIONS.includes(i))" class="flex flex-wrap gap-1.5 mt-2">
              <span v-for="it in selectedInterests.filter(i => !INTEREST_OPTIONS.includes(i))" :key="it"
                class="inline-flex items-center gap-1 text-xs bg-amber-50 text-amber-700 border border-amber-200 px-2.5 py-1 rounded-full">
                {{ it }}
                <button type="button" class="hover:text-red-500" @click="toggleInterest(it)">×</button>
              </span>
            </div>
          </div>
        </div>
